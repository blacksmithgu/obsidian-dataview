...
export type CompareImpl<T extends LiteralValue> = (
    first: T,
    second: T
) => number;
...
export class BinaryOpHandler {
    ...
    public compare<T extends LiteralTypeOrAll>(
        type: T,
        compare: CompareImpl<LiteralReprAll<T>>
    ): BinaryOpHandler {
        return this.register(type, '<', type, (a, b) => compare(a, b) < 0)
    ...
    public evaluate(
        ...
        const handler = this.map.get(
            BinaryOpHandler.repr(op, leftType, rightType)
        );
        if (handler) return Result.success(handler(left, right));

        // Right-'*' fallback:
        const handler2 = this.map.get(BinaryOpHandler.repr(op, leftType, '*'));
        if (handler2) return Result.success(handler2(left, right));

        // Left-'*' fallback:
        const handler3 = this.map.get(BinaryOpHandler.repr(op, '*', rightType));
        if (handler3) return Result.success(handler3(left, right));

        // Double '*' fallback.
        const handler4 = this.map.get(BinaryOpHandler.repr(op, '*', '*'));
        if (handler4) return Result.success(handler4(left, right));

        return Result.failure(
            `Operator '${op}' is not supported for '${leftType}' and '${rightType}`
        );
    }
    ...
    return (
        BinaryOpHandler.create()
            ...
            .register('array', '+', 'array', (a, b) =>
                ([] as LiteralValue[]).concat(a).concat(b)
            )
            // Object operations.
            .register('object', '+', 'object', (a, b) =>
                Object.assign({}, a, b)
            )
    );
}
