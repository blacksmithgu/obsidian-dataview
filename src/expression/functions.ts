...
export class FunctionBuilder {
    ...
    public add2<T extends LiteralTypeOrAll, U extends LiteralTypeOrAll>(
        ...
        this.variants.push({
            ...
            impl: (c, ...rest) =>
                impl(
                    rest[0] as LiteralReprAll<T>,
                    rest[1] as LiteralReprAll<U>,
                    c
                ),
        });
        return this;
    ...
    public build(): FunctionImpl {
        const self: FunctionImpl = (
            context: Context,
            ...args: LiteralValue[]
        ) => {
            const types: LiteralType[] = [];
            for (const arg of args) {
                const argType = Values.typeOf(arg);
                if (!argType)
                    throw Error(
                        `Unrecognized argument type for argument '${arg}'`
                    );
                types.push(argType);
            }
            ...
            if (this.vectorized[types.length]) {
                for (const vec of this.vectorized[types.length]) {
                    if (types[vec] != 'array') continue;

                    return (args[vec] as LiteralValue[]).map(v => {
                        const newArgs = ([] as LiteralValue[]).concat(args);
                        newArgs[vec] = v;
            ...
            outer: for (const variant of this.variants) {
                ...
                for (let index = 0; index < variant.args.length; index++) {
                    if (
                        variant.args[index] != '*' &&
                        variant.args[index] != types[index]
                    )
                        continue outer;
                }
            ...
            throw Error(
                `No implementation of '${
                    this.name
                }' found for arguments: ${types.join(', ')}`
            );
        };
...
export namespace Functions {
    ...
    export function bindAll(
        ...
        const result: Record<string, BoundFunctionImpl> = {};
...
export namespace DefaultFunctions {
    ...
    export const date = new FunctionBuilder('date')
        .add1('string', str => {
            const parsedDate = EXPRESSION.date.parse(str);
        ...
        .add1('link', (link, c) => {
            ...
            if (link.display) {
                const parsedDate = EXPRESSION.date.parse(link.display);
                if (parsedDate.status) return parsedDate.value;
            }

            // Then try to parse from the path...
            const parsedDate = EXPRESSION.date.parse(link.path);
            if (parsedDate.status) return parsedDate.value;

            // Then pull it from the file.
            const resolved = c.linkHandler.resolve(link.path);
    ...
    export const striptime = new FunctionBuilder('striptime')
        .add1('date', d =>
            DateTime.fromObject({year: d.year, month: d.month, day: d.day})
        )
        .add1('null', _n => null)
        .vectorize(1, [0])
        .build();
    ...
    export const contains: FunctionImpl = new FunctionBuilder('contains')
        ...
        .add2('*', '*', (elem1, elem2, context) =>
            context
                .evaluate(
                    Fields.binaryOp(
                        Fields.literal(elem1),
                        '=',
                        Fields.literal(elem2)
                    )
                )
                .orElseThrow()
        )
    ...
    export const extract: FunctionImpl = (
        context: Context,
        ...args: LiteralValue[]
    ) => {
        if (args.length == 0)
        ...
        if (Values.isArray(object))
            return object.map(v => extract(context, v, ...args.slice(1)));
        ...
        for (let index = 1; index < args.length; index++) {
            ...
            result[key] = context
                .evaluate(
                    Fields.index(Fields.literal(object), Fields.literal(key))
                )
                .orElseThrow();
        }
    ...
    export const sort = new FunctionBuilder('sort')
        .add1('array', (list, context) => {
            const result = ([] as LiteralValue[]).concat(list);
            result.sort((a, b) => {
                const le = context
                    .evaluate(
                        Fields.binaryOp(
    ...
    export const regexreplace = new FunctionBuilder('regexreplace')
        .add3(
            ...
            'string',
            'string',
            (field: string, pat: string, rep: string) => {
                try {
    ...
    export const join: FunctionImpl = new FunctionBuilder('join')
        .add2('array', 'string', (arr, sep) =>
            arr.map(e => Values.toString(e)).join(sep)
        )
