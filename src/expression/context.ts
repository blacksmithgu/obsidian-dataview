...
export class Context {
    ...
    public constructor(
        ...
        public binaryOps: BinaryOpHandler = createBinaryOps(
            linkHandler.normalize
        ),
    ...
    public tryEvaluate(
        field: Field,
        data: Record<string, LiteralValue> = {}
    ): LiteralValue {
    ...
    public evaluate(
        ...
        switch (field.type) {
            ...
            case 'negated':
                return this.evaluate(field.child, data).map(
                    s => !Values.isTruthy(s)
                );
            case 'binaryop':
            ...
            case 'function':
                ...
                const func = rawFunc.value;

                const args: LiteralValue[] = [];
                for (const arg of field.arguments) {
                    const resolved = this.evaluate(arg, data);
                ...
                if (Values.isFunction(func)) call = func as FunctionImpl;
                    ...
                    return Result.failure(
                        `Unrecognized function name '${func}'`
                    );
                else
                    return Result.failure(
                        `Cannot call type '${Values.typeOf(
                            func
                        )}' as a function`
                    );
            ...
            case 'index':
                ...
                const checkedIndex: Result<string | number, string> =
                    literalIndex.flatMap(s =>
                        Values.isString(s) || Values.isNumber(s)
                            ? Result.success<string | number, string>(s)
                            : Result.failure(
                                  'Can only index with a string, variable, or number'
                              )
                    );
                ...
                const checkedObject =
                    field.object.type == 'variable' &&
                    field.object.name == 'row'
                        ? Result.success<LiteralValue, string>(
                              Object.assign({}, this.globals, data)
                          )
                        : this.evaluate(field.object, data);
                if (!checkedObject.successful) return checkedObject;

                const object = Values.wrapValue(checkedObject.value);
                if (!object)
                    return Result.failure(
                        'Unrecognized object to index into: ' + object
                    );
                ...
                switch (object.type) {
                    ...
                    case 'link':
                        ...
                        const linkValue = this.linkHandler.resolve(
                            object.value.path
                        );
                        ...
                        switch (index) {
                            ...
                            case 'week':
                                return Result.success(
                                    Math.floor(object.value.day / 7) + 1
                                );
                            case 'weekday':
