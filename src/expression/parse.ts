...
export function createBinaryParser<T, U>(
    ...
    return P.seqMap(
        child,
        P.seq(P.optWhitespace, sep, P.optWhitespace, child).many(),
        (first, rest) => {
    ...
    return P.custom((success, failure) => {
        return (input, i) => {
            ...
            for (const func of funcs) {
                const next = (func(result.value as T) as any)._(
                    input,
                    result.index
                );
...
export const EXPRESSION = P.createLanguage<ExpressionLanguage>({
    ...
    tag: q =>
        P.regexp(/#[\p{Letter}\p{Emoji_Presentation}\w/-]+/u).desc(
            "tag ('#hello/stuff')"
        ),
    ...
    identifier: q =>
        P.regexp(
            /[\p{Letter}\p{Emoji_Presentation}][\p{Letter}\p{Emoji_Presentation}\w_-]*/u
    ...
    date: q =>
        chainOpt<DateTime>(
            q.rootDate,
            (ym: DateTime) =>
                P.seqMap(P.string('-'), P.regexp(/\d{2}/), (_, day) =>
                    ym.set({day: Number.parseInt(day)})
                ),
            (ymd: DateTime) =>
                P.seqMap(P.string('T'), P.regexp(/\d{2}/), (_, hour) =>
                    ymd.set({hour: Number.parseInt(hour)})
                ),
            (ymdh: DateTime) =>
            ...
            (dt: DateTime) =>
                P.seqMap(
                    P.string('+').or(P.string('-')),
                    P.regexp(/\d{1,2}(:\d{2})?/),
                    (pm, hr) => dt.setZone('UTC' + pm + hr)
                )
        ),
    ...
    // A duration of time.
    durationType: q =>
        P.alt(...Object.keys(DURATION_TYPES).map(P.string)) as P.Parser<
            keyof typeof DURATION_TYPES
        >,
    duration: q =>
        P.seqMap(
            q.number,
            ...
            q.durationType,
            P.string('s').atMost(1),
            (count, _, t, _2) => DURATION_TYPES[t].mapUnits(x => x * count)
        ),
    ...
    linkOutgoingSource: q =>
        P.seqMap(
            P.string('outgoing(').skip(P.optWhitespace),
            q.link,
            P.string(')'),
    ...
    inlineFieldList: q =>
        q.atomInlineField.sepBy(
            P.string(',').trim(P.optWhitespace).lookahead(q.atomInlineField)
        ),
    inlineField: q =>
        P.alt(
            P.seqMap(
    ...
    indexField: q =>
        P.seqMap(
            q.atomField,
            P.alt(q.dotPostfix, q.indexPostfix, q.functionPostfix).many(),
            (obj, postfixes) => {
                let result = obj;
    ...
    negatedField: q =>
        P.seqMap(P.string('!'), q.indexField, (_, field) =>
            Fields.negate(field)
        ).desc('negated field'),
    parensField: q =>
        P.seqMap(
