...
test('Parse String Literal', () => {
    expect(EXPRESSION.string.parse('this won\'t work, no quotes').status).toBe(
        false
    );
...
test('Parse Multiple Strings', () => {
    const result = EXPRESSION.field.tryParse("\"\" or \"yes\"") as BinaryOpField;
    expect(result.type).toBe('binaryop');

    const left = result.left as LiteralField;
    expect(left.type).toBe('literal');
    expect(left.value).toBe('');

    const right = result.right as LiteralField;
...
test('Parse Year-Month date', () => {
    const date = EXPRESSION.date.tryParse("2020-04");
    expect(date.year).toBe(2020);
    expect(date.month).toBe(4);
});

test('Parse Year-Month-Day date', () => {
    const date = EXPRESSION.date.tryParse("1984-08-15");
...
test('Parse Year-Month-DayTHour:Minute:Second', () => {
    const date = EXPRESSION.date.tryParse("1984-08-15T12:42:59");
    expect(date.year).toBe(1984);
    expect(date.month).toBe(8);
    expect(date.day).toBe(15);
...
test('Parse Year-Month-DayTHour:Minute:Second-Offset', () => {
    const date = EXPRESSION.date.tryParse("1984-08-15T12:40:50-07:00");
    expect(date.zoneName).toBe('UTC-7');

    const date2 = EXPRESSION.date.tryParse("1984-08-15T12:40:50+9");
    expect(date2.zoneName).toBe('UTC+9');

    const date3 = EXPRESSION.date.tryParse("1985-12-06T19:40:10+06:30");
    expect(date3.zoneName).toBe('UTC+6:30');
});

test('Parse Today', () => {
    const date = EXPRESSION.dateField.tryParse("date(today)") as LiteralField;
...
test('Parse day duration', () => {
    const day = EXPRESSION.duration.tryParse("6 days");
    const day2 = EXPRESSION.duration.tryParse("6day");
...
test('Parse minute duration', () => {
    const min = EXPRESSION.duration.tryParse("4min");
    const min2 = EXPRESSION.duration.tryParse("4 minutes");
    const min3 = EXPRESSION.duration.tryParse("4 minute");
...
test('Parse Simple Addition', () => {
    const result = EXPRESSION.binaryOpField.parse("16 + \"what\"") as Success<BinaryOpField>;
    expect(result.status).toBe(true);
    expect(result.value).toEqual(
        Fields.binaryOp(Fields.literal(16), '+', Fields.literal('what'))
    );
});
...
test('Parse Parenthesis', () => {
    const result = EXPRESSION.field.parse("(16 - 4) - 8") as Success<BinaryOpField>;
    expect(result.status).toBe(true);
    expect(result.value).toEqual(
        Fields.binaryOp(
            Fields.binaryOp(Fields.literal(16), '-', Fields.literal(4)),
            '-',
            Fields.literal(8)
        )
    );
...
test('Parse binaryop negated field', () => {
    ...
    expect(EXPRESSION.field.tryParse('true & !false')).toEqual(
        Fields.binaryOp(
            Fields.literal(true),
            '&',
            Fields.negate(Fields.literal(false))
        )
    );
});
...
test('Parse negated parens source', () => {
    ...
    expect(EXPRESSION.source.tryParse('-"meme" & #dirty')).toEqual(
        Sources.binaryOp(
            Sources.negate(Sources.folder('meme')),
            '&',
            Sources.tag('#dirty')
        )
    );
