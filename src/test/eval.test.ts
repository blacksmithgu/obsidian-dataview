...
test('Evaluate simple numeric operations', () => {
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal(2), '+', Fields.literal(4))
        )
    ).toEqual(6);
    expect(
        simpleContext().tryEvaluate(
    ...
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal(8), '/', Fields.literal(4))
        )
    ).toEqual(2);
});
...
test('Evaluate numeric comparisons', () => {
    ...
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal(-2), '=', Fields.literal(-2))
        )
    ).toEqual(true);
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal(-2), '>=', Fields.literal(-8))
        )
    ).toEqual(true);
});
...
test('Evaluate simple string operations', () => {
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal('a'), '+', Fields.literal('b'))
        )
    ).toEqual('ab');
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal('a'), '+', Fields.literal(12))
        )
    ).toEqual('a12');
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal('a'), '*', Fields.literal(6))
        )
    ).toEqual('aaaaaa');
});
...
test('Evaluate string comparisons', () => {
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal('abc'), '<', Fields.literal('abd'))
        )
    ).toEqual(true);
    expect(
        simpleContext().tryEvaluate(
            Fields.binaryOp(Fields.literal('xyz'), '=', Fields.literal('xyz'))
        )
    ).toEqual(true);
});
...
test('Evaluate date comparisons', () => {
    ...
    expect(
        parseEval('contains(list(date(2020-01-01)), date(2020-01-01))')
    ).toEqual(true);
});

test('Evaluate date subtraction', () => {
    const duration = parseEval(
        'date(2021-05-04) - date(1997-05-17)'
    ) as Duration;
...
test('Evaluate simple field resolution', () => {
    const context = simpleContext().set('a', 18).set('b', 'hello');
...
test('Evaluate simple object resolution', () => {
    const object = {inner: {final: 6}};
    const context = simpleContext().set('obj', object);

    expect(context.tryEvaluate(Fields.indexVariable('obj.inner'))).toEqual(
        object.inner
    );
    expect(
        context.tryEvaluate(Fields.indexVariable('obj.inner.final'))
    ).toEqual(object.inner.final);
});
...
test('Evaluate simple link resolution', () => {
    ...
    expect(context.tryEvaluate(Fields.indexVariable('link.inner'))).toEqual(
        object.inner
    );
    expect(
        context.tryEvaluate(Fields.indexVariable('link.inner.final'))
    ).toEqual(object.inner.final);
});

/** Parse a field expression and evaluate it in the simple context. */
function parseEval(text: string): LiteralValue {
    const field = EXPRESSION.field.tryParse(text);
