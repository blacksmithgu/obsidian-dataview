...
test('Parse commas inside inline field', () => {
    expect(EXPRESSION.inlineField.tryParse('[[yes, no, and maybe]]')).toEqual(
        Link.file('yes, no, and maybe')
    );
});
...
test('Parse basic inline fields', () => {
    ...
    expect(EXPRESSION.inlineField.tryParse('[[test]]')).toEqual(
        Link.file('test')
    );
});
...
test('Parse inline field lists', () => {
    ...
    expect(EXPRESSION.inlineField.tryParse('[[test]], [[test2]]')).toEqual([
        Link.file('test'),
        Link.file('test2'),
    ]);
    expect(EXPRESSION.inlineField.tryParse('1, 2, 3, "hello"')).toEqual([
        1,
        2,
        3,
        'hello',
