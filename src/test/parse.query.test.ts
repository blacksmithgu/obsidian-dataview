...
test('Parse Query Type', () => {
    const unknown = QUERY_LANGUAGE.queryType.parse('vehicle');
    expect(unknown.status).toBe(false);

    const list = QUERY_LANGUAGE.queryType.tryParse('list');
    expect(list).toEqual('list');

    const listUpper = QUERY_LANGUAGE.queryType.tryParse('lIsT');
...
test('Named Fields', () => {
    const simple = QUERY_LANGUAGE.namedField.tryParse('time-played');
    expect(simple).toEqual(
        QueryFields.named('time-played', Fields.variable('time-played'))
    );

    const complex = QUERY_LANGUAGE.namedField.tryParse(
        '(time-played + 4) as something'
    );
    expect(complex).toEqual(
        QueryFields.named(
            'something',
            Fields.binaryOp(
                Fields.variable('time-played'),
                '+',
                Fields.literal(4)
            )
        )
...
test('Sort Fields', () => {
    const simple = QUERY_LANGUAGE.sortField.tryParse('time-played DESC');
    expect(simple).toEqual(
        QueryFields.sortBy(Fields.variable('time-played'), 'descending')
    );

    const complex = QUERY_LANGUAGE.sortField.tryParse(
        '(time-played - "where")'
    );
    expect(complex).toEqual(
        QueryFields.sortBy(
...
test('Fat Query', () => {
    ...
    expect(fat.source).toEqual(
        Sources.binaryOp(Sources.tag('#games'), '|', Sources.tag('#gaming'))
    );
    expect((fat.operations[1] as SortByStep).fields.length).toBe(2);
});

test('List query with format', () => {
    const query = parseQuery('LIST file.name FROM #games').orElseThrow();
    expect(query.header.type).toBe('list');
    expect((query.header as ListQuery).format).toEqual(
        Fields.indexVariable('file.name')
    );
});

test('Task query with no fields', () => {
    const q = parseQuery('task from #games').orElseThrow();
