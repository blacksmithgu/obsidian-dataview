...
test('Evaluate replace(list, string, string)', () => {
    expect(parseEval('replace(list("yes", "re"), "e", "a")')).toEqual([
        'yas',
        'ra',
    ]);
});
...
test('Evaluate reverse(list)', () => {
    expect(parseEval('reverse(list(1, 2, 3))')).toEqual(
        parseEval('list(3, 2, 1)')
    );
    expect(parseEval('reverse(list("a", "b", "c"))')).toEqual(
        parseEval('list("c", "b", "a")')
    );
});
...
test('Evaluate sort(list)', () => {
    expect(parseEval('sort(list(2, 3, 1))')).toEqual(
        parseEval('list(1, 2, 3)')
    );
    expect(parseEval('sort(list("a", "c", "b"))')).toEqual(
        parseEval('list("a", "b", "c")')
    );
});
...
test('Evaluate vectorized all()', () => {
    ...
    expect(parseEval('all(regexmatch("a+", list("a", "aaab")))')).toEqual(
        false
    );
...
test('Evaluate 1 field extract()', () => {
    const res = parseEval('extract(object("mtime", 1), "mtime")');
    expect(res).toEqual({mtime: 1});
});

test('Evaluate 2 field extract()', () => {
    const res = parseEval(
        'extract(object("mtime", 1, "yes", "hello"), "yes", "mtime")'
    );
    expect(res).toEqual({
        yes: 'hello',
        mtime: 1,
...
test('Evaluate date()', () => {
    expect(parseEval('date([[2020-04-18]])')).toEqual(
        DateTime.fromObject({year: 2020, month: 4, day: 18})
    );
    expect(parseEval('date([[Place|2021-04]])')).toEqual(
        DateTime.fromObject({year: 2021, month: 4, day: 1})
    );
});
...
test('Evaluate nonnull()', () => {
    expect(DefaultFunctions.nonnull(simpleContext(), null, null, 1)).toEqual([
        1,
    ]);
