...
const LINE_KEY_PARSER = P.alt(P.regex(new RegExp(emojiRegex(), "")), P.regex(/[-0-9\p{Letter}\w\s]+/u))
    .wrap(P.oneOf("(*~_`"), P.oneOf(")*~_`"))
    .many()
    .map(result => result.join(""));

/**
 * Simple implementation for extracting a full-line inline field; this does not handle any complicated
 * multi-line cases since those require more work in the markdown parser.
 */
export function extractFullLineField(line: string): InlineField | undefined {
    if (!line.contains("::")) return undefined;
    let splitLocation = line.indexOf("::");
    let maybeKey = LINE_KEY_PARSER.parse(line.substring(0, splitLocation).trim());

    if (!maybeKey.status) return undefined;
