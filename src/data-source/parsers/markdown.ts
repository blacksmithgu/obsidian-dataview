...
/** A list element inside of a markdown list. */
export interface MarkdownListElement {
    /** The symbol used to define the list element ('*' or '-' or '+' or '1.') */
    symbol: string;
    /** If set, then this element is a task and the value is the marker between the brackets ('[]'). */
    task?: string;
    /** The blocks that immediately follow the '-' sign; usually a paragraph element with potential for other elements. */
    text: MarkdownBlock[];
    /** The line that this list element first starts on. */
    line: number;
    /** All of the contents after the text of this list element; will generally start with a list. */
    contents: MarkdownBlock[];
    /** If present, the block ID pointing to this element. */
    blockId?: string;
}

/** A markdown block inside of a markdown document. */
export type MarkdownBlock =
    | { type: "paragraph"; contents: string[]; line: number; blockId?: string }
    ...
    | { type: "rule"; symbol: string; count: number; line: number }
...
const LIST_ELEMENT = /^([-+*]|\d+\.)\s*(\[.?\])?\s*(.*)$/u;

/** Compute the last line of a markdown block. */
export function markdownBlockEndingLine(block: MarkdownBlock): number {
    switch (block.type) {
        case "rule":
            return block.line;
        case "frontmatter":
        case "codeblock":
            return block.line + block.contents.length + 1;
        case "paragraph":
        case "blockquote":
            return block.line + block.contents.length - 1;
        case "heading":
            return block.line + (block.ruling ? 1 : 0);
        case "list":
            let lastElement = block.elements[block.elements.length - 1];
            if (lastElement.contents)
                return markdownBlockEndingLine(lastElement.contents[lastElement.contents.length - 1]);
            else return markdownBlockEndingLine(lastElement.text[lastElement.text.length - 1]);
    }
}

function markdownToRawIndent(text: string, excludeFirst: boolean): string {
    return text
        .split("\n")
        .map((value, index) => {
            if (excludeFirst && index == 0) return value;
            return "    " + value;
        })
        .join("\n");
}

export function markdownToRaw(blocks: MarkdownBlock[] | MarkdownBlock): string {
    if (Array.isArray(blocks)) return blocks.map(b => markdownToRaw(b)).join("\n\n");

    switch (blocks.type) {
        case "rule":
            return blocks.symbol.repeat(blocks.count);
        case "paragraph":
            return blocks.contents.join("\n");
        case "heading":
            return blocks.ruling ? blocks.text + "\n===" : "#".repeat(blocks.level) + " " + blocks.text;
        case "frontmatter":
            return "---\n" + blocks.contents.join("\n") + "\n---";
        case "codeblock":
            return (
                blocks.delimiter +
        ...
        case "blockquote":
            return blocks.contents.map(l => "> " + l).join("\n");
        case "list":
            let result = "";
            for (let element of blocks.elements) {
                result +=
                    element.symbol +
                    " " +
                    (element.task ? "[" + element.task + "] " : "") +
                    markdownToRawIndent(markdownToRaw(element.text), true) +
                    "\n";
                if (element.contents) result += "\n" + markdownToRawIndent(markdownToRaw(element.contents), false);
            }
            return result;
    }
}
...
export function markdownBlocks(
    ...
    while (tokenizer.peek() !== undefined && emptyLineOrIndented(tokenizer.peek()!, requiredIndent)) {
        ...
        // The '===' case is handled by the general text handler.
            ...
            blocks.push({ type: "rule", symbol: line.symbol, count: line.count, line: lineno });
            continue;
        }
    ...
    if (tokenizer.peek() !== undefined && classifyLine(tokenizer.peek()!).type == "heading-ruling") {
        ...
        if (contents.length > 1)
            result.push({
                ...
                line: lineno,
                blockId: extractBlockId(contents.slice(0, -2)),
            });
        result.push({
            type: "heading",
            level: 1,
        ...
        return [{ type: "paragraph", contents, line: lineno, blockId: extractBlockId(contents) }];
    }
}
...
export function markdownListElement(
    ...
    let initialParagraph = {
        type: "paragraph",
        contents: followingText,
        line: lineno,
        blockId: extractBlockId(followingText),
    } as MarkdownBlock;
    ...
    for (let index = 0; index < subblocks.length && subblocks[index].type != "list"; index++)
        prefix.push(subblocks[index]);

    return {
        symbol,
        contents: subblocks.slice(prefix.length),
        task,
        text: [initialParagraph].concat(prefix),
        line: lineno,
    };
}
...
// Broadly matches links of the form (!)[[(stuff)]]; the actual parsing is done by the link parser.
const POTENTIAL_LINK = /!?\[\[[^\[\]]+\]\]/giu;

/** Extract links from a line in the document. */
export function extractLinks(
    line: string
): { value: Link; col: number; length: number; style: "markdown" | "obsidian" }[] {
...
// Matches tags of the form '#thing/or/thing'; this is a broad match which is refined by the actual tag parser.
const POTENTIAL_TAG = /#[^#\s]+/giu;

/** Extract tags from a line in the document. */
export function extractTags(line: string): { value: string; col: number; length: number }[] {
    ...
    for (let match of matches) {
        ...
        if (maybeTag.status) result.push({ value: maybeTag.value, col: match.index!, length: match[0].length });
    }
...
export const BLOCK_ID_REGEX = /\^(\S+)\s+$/iu;

/** Extract a block ID from a line if present. */
export function extractBlockId(line: string | string[]): string | undefined {
    if (Array.isArray(line)) return line.find(l => extractBlockId(l));

    let match = line.match(BLOCK_ID_REGEX);
