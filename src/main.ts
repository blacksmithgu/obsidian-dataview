...
export default class DataviewPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.registerMarkdownCodeBlockProcessor(
            ...
            async (source: string, el, ctx) => {
                const maybeQuery = tryOrPropogate(() =>
                    parseQuery(source, this.settings)
                );
        ...
        this.registerMarkdownPostProcessor(async (el, ctx) => {
            ...
            for (let index = 0; index < codeblocks.length; index++) {
                ...
                if (text.startsWith(this.settings.inlineJsQueryPrefix)) {
                    ...
                    const potentialField = text
                        .substring(this.settings.inlineQueryPrefix.length)
                        .trim();

                    const field = tryOrPropogate(() =>
                        parseField(potentialField)
                    );
                    if (!field.successful) {
                        const errorBlock = el.createEl('div');
                        renderErrorPre(
                            errorBlock,
    ...
    async prepareIndexes() {
        const index = await FullIndex.generate(
            this.app.vault,
...
class DataviewSettingsTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(this.containerEl)
            ...
            .setDesc(
                'What null/non-existent should show up as in tables, by default.'
            )
            .addText(text =>
                text
                    .setPlaceholder('-')
        ...
        new Setting(this.containerEl)
            ...
            .setDesc(
                'How frequently dataviews are updated in preview mode when files are changing.'
            )
            .addText(text =>
                text
                    .setPlaceholder('5000')
        ...
        new Setting(this.containerEl)
            ...
            .addToggle(toggle =>
                toggle.setValue(this.plugin.settings.enableDataviewJs).onChange(
                    async value =>
                        await this.plugin.updateSettings({
                            enableDataviewJs: value,
                        })
                )
            );
    }
...
class EnsurePredicateRenderer extends MarkdownRenderChild {
    ...
    async onload() {
        const loadContainer = renderErrorPre(
            this.container,
            'Dataview indices are loading'
        );
...
class DataviewListRenderer extends MarkdownRenderChild {
    ...
    async render() {
        const maybeResult = tryOrPropogate(() =>
            executeList(this.query, this.index, this.origin)
        );
...
class DataviewTableRenderer extends MarkdownRenderChild {
    ...
    async render() {
        const maybeResult = tryOrPropogate(() =>
            executeTable(this.query, this.index, this.origin)
        );
        if (!maybeResult.successful) {
            renderErrorPre(this.container, 'Dataview: ' + maybeResult.error);
            return;
        ...
        const result = maybeResult.value;
        const dataWithNames: LiteralValue[][] = [];
        for (const entry of result.data) {
            dataWithNames.push([entry.id].concat(entry.values));
        }
        const name = result.idMeaning.type === 'group' ? 'Group' : 'File';
...
class DataviewTaskRenderer extends MarkdownRenderChild {
    ...
    async render() {
        const result = tryOrPropogate(() =>
            executeTask(this.query, this.origin, this.index)
        );
        if (!result.successful) {
            renderErrorPre(this.container, 'Dataview: ' + result.error);
        } else if (
            result.value.tasks.size == 0 &&
            this.settings.warnOnEmptyResult
        ) {
            ...
            this.addChild(
                (this.taskView = new Tasks.TaskViewLifecycle(
                    this.vault,
                    this.container
                ))
            );
...
class DataviewInlineRenderer extends MarkdownRenderChild {
    ...
    async render() {
        const result = tryOrPropogate(() =>
            executeInline(this.field, this.origin, this.index)
        );
        if (!result.successful) {
            this.errorbox = this.container.createEl('div');
            renderErrorPre(
                this.errorbox,
                "Dataview (for inline query '" +
                    this.fieldText +
                    "'): " +
                    result.error
            );
