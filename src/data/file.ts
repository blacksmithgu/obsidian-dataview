...
export namespace Task {
    ...
    export function copy(input: Task): Task {
        const partial = Object.assign({}, input) as Task;
...
export class PageMetadata {
    ...
    public fullTags(): Set<string> {
        ...
        for (const tag of this.tags) {
            for (const subtag of PageMetadata.parseSubtags(tag))
                result.add(subtag);
        }
    ...
    public toObject(index: FullIndex): Record<string, LiteralValue> {
        ...
        const result: Record<string, LiteralValue> = {
            file: {
                ...
                ctime: this.ctime,
                cday: DateTime.fromObject({
                    year: this.ctime.year,
                    month: this.ctime.month,
                    day: this.ctime.day,
                }),
                mtime: this.mtime,
                mday: DateTime.fromObject({
                    year: this.mtime.year,
                    month: this.mtime.month,
                    day: this.mtime.day,
                }),
                size: this.size,
                ext: this.extension(),
        ...
        if (this.day)
            (result['file'] as Record<string, LiteralValue>)['day'] = this.day;
...
export function toTransferable(parsed: ParsedMarkdown): TransferableMarkdown {
    const newFields = new Map<string, TransferableValue[]>();
    for (const [key, values] of parsed.fields.entries()) {
        newFields.set(
            key,
...
export function fromTransferable(parsed: TransferableMarkdown): ParsedMarkdown {
    const newFields = new Map<string, LiteralValue[]>();
    for (const [key, values] of parsed.fields.entries()) {
        newFields.set(
            key,
...
function findDate(
    ...
    for (const key of fields.keys()) {
        if (
            !(
                key.toLocaleLowerCase() == 'date' ||
                key.toLocaleLowerCase() == 'day'
            )
        )
            continue;

        const value = fields.get(key) as LiteralValue;
...
export function parseFrontmatter(value: any): LiteralValue {
    if (value == null) {
        ...
        if (Array.isArray(value)) {
            const object = value as Array<any>;
            ...
            if (
                ...
                object[0].every(v => typeof v === 'string')
            ) {
        ...
        const dateParse = EXPRESSION.date.parse(value);
        if (dateParse.status) return dateParse.value;

        const durationParse = EXPRESSION.duration.parse(value);
        if (durationParse.status) return durationParse.value;

        const linkParse = EXPRESSION.embedLink.parse(value);
...
export function parseInlineField(value: string): LiteralValue {
    ...
    // separately.
    const inline = EXPRESSION.inlineField.parse(value);
...
export function addInlineField(
    ...
    if (fields.has(name)) {
        const existing = fields.get(name) as LiteralValue;
...
export function taskAny(t: Task, f: (t: Task) => boolean): boolean {
    if (f(t)) return true;
    for (const sub of t.subtasks) if (taskAny(sub, f)) return true;
...
export function findTasksInFile(path: string, file: string): Task[] {
    ...
    let lineno = 0;
    for (const line of file.replace('\r', '').split('\n')) {
        lineno += 1;
        ...
        const indent = match[1].replace('\t', '    ').length;
        const isReal = !!match[2] && match[2].trim().length > 0;
        const isCompleted = !isReal || match[2] == '[X]' || match[2] == '[x]';
        const task: Task = {
            text: match[3],
...
export async function parseMarkdown(
    ...
    const fields: Map<string, LiteralValue[]> = new Map();
    ...
    for (let line of contents.split('\n')) {
        ...
        const match = inlineRegex.exec(line);
        if (!match) continue;

        const name = match[1].trim();
        const inlineField = parseInlineField(match[2]);

        fields.set(name, (fields.get(name) ?? []).concat([inlineField]));
        const simpleName = canonicalizeVarName(match[1].trim());
        if (simpleName.length > 0 && simpleName != match[1].trim()) {
            fields.set(
                simpleName,
                (fields.get(simpleName) ?? []).concat([inlineField])
            );
        }
    }

    // And extract tasks...
    const tasks = findTasksInFile(path, contents);
...
export function parsePage(
    ...
    if (fileCache) {
        ...
        if (fileCache.frontmatter) {
            const frontTags = parseFrontMatterTags(fileCache.frontmatter);
            if (frontTags) {
            ...
            const frontAliases = parseFrontMatterAliases(fileCache.frontmatter);
            if (frontAliases) {
                for (const alias of frontAliases) aliases.add(alias);
            }

            const frontFields = parseFrontmatter(
                fileCache.frontmatter
            ) as Record<string, LiteralValue>;
            for (const [key, value] of Object.entries(frontFields))
    ...
    if (file.path in cache.resolvedLinks) {
        for (const resolved in cache.resolvedLinks[file.path]) {
            links.push({
                type: 'file',
                path: resolved,
                display: resolved,
    ...
    for (const [name, values] of markdownData.fields.entries()) {
        for (const value of values) addInlineField(fields, name, value);
    }
