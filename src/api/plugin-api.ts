...
export class DataviewApi {
    ...
    public pagePaths(query?: string, originFile?: string): DataArray<string> {
        ...
        return collectPagePaths(source, this.index, originFile)
            .map(s => DataArray.from(s))
            .orElseThrow();
    }
    ...
    public page(
        ...
        const normPath = this.app.metadataCache.getFirstLinkpathDest(
            rawPath,
            originFile ?? ''
        );
        if (!normPath) return undefined;

        const pageObject = this.index.pages.get(normPath.path);
    ...
    public pages(query?: string, originFile?: string): DataArray<any> {
        return this.pagePaths(query, originFile).flatMap(p => {
            const res = this.page(p, originFile);
            return res ? [res] : [];
        });
    }
    ...
    public taskList(
        ...
        if (groupByFile) {
            ...
            const subcontainer = container.createDiv();
            (async () => {
                await renderFileTasks(subcontainer, byFile);
                component.addChild(
                    new TaskViewLifecycle(this.app.vault, subcontainer)
                );
            })();
        } else {
            const subcontainer = container.createDiv();
