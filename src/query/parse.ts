...
export const QUERY_LANGUAGE = P.createLanguage<QueryLanguageTypes>({
    ...
    namedField: q =>
        P.alt<NamedField>(
            ...
            EXPRESSION.identifierDot.map(ident =>
                QueryFields.named(ident, Fields.indexVariable(ident))
            )
        ),
    sortField: q =>
        P.seqMap(
            ...
            (field, dir) => {
                let direction =
                    dir.length == 0 ? 'ascending' : dir[0].toLowerCase();
            ...
            switch (qtype) {
                case 'table':
                    return P.sepBy(
                        ...
                        return {type: 'table', fields} as QueryHeader;
                    });
                case 'list':
    ...
    flattenClause: q =>
        P.seqMap(
            P.regexp(/FLATTEN/i).skip(P.whitespace),
            q.namedField,
            (_, field) => {
    ...
    groupByClause: q =>
        P.seqMap(
            P.regexp(/GROUP BY/i).skip(P.whitespace),
            q.namedField,
            (_, field) => {
...
export function parseQuery(
    ...
    try {
        const query = QUERY_LANGUAGE.query.tryParse(text);
