...
export async function renderCompactMarkdown(
    ...
    if (subcontainer.children.length == 1 && subcontainer.querySelector('p')) {
        subcontainer.innerHTML =
            subcontainer.querySelector('p')?.innerHTML ?? '';
    }
}
...
export async function renderTable(
    ...
    const theadEl = tableEl.createEl('thead', {cls: 'table-view-thead'});
    const headerEl = theadEl.createEl('tr', {cls: 'table-view-tr-header'});
    for (const header of headers) {
        headerEl.createEl('th', {text: header, cls: 'table-view-th'});
    }

    const tbodyEl = tableEl.createEl('tbody', {cls: 'table-view-tbody'});
...
export function renderErrorSpan(
    container: HTMLElement,
    error: string
): HTMLElement {
    const pre = container.createEl('span', {
        cls: ['dataview', 'dataview-error'],
    });
    pre.appendText(error);
    return pre;
