...
import { parseCsv } from "data-source/parsers/csv";
import { FileImporter } from "data-source/import/import-manager";
...
export class FullIndex {
    ...
    /** Destroy any unneeded listeners */
    public terminate() {}

    /** Queue a file for reloading; this is done asynchronously in the background and may take a few seconds. */
    public reload(file: TFile) {
        this.importer.reload<PageMetadata>(file).then(r => this.reloadInternal(file, r));
    }
    ...
    private reloadInternal(file: TFile, meta: PageMetadata) {
        this.pages.set(file.path, meta);
        this.tags.set(file.path, meta.fullTags());
        this.etags.set(file.path, meta.exactTags());
        this.links.set(file.path, new Set<string>(meta.links.map(l => l.value.path)));
